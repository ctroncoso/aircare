...
- [x] **10. Status topic (v1.1.15+)**: config/state + health moved off `cleanair/sensor` to a dedicated **`cleanair/status`** topic → InfluxDB **`status`** measurement (fields: `sched_mode`, `sched_ovr`, `sched_exc`, `exc1_from/to/state`, `exc2_from/to/state`, `relay`, `relay2`, `relay_both`, `rssi`, `uptime_ms`, `heap`, `local_time`; tags `mac`/`label`/`fw`). The standalone `HEALTH` heartbeat was retired — `publishStatus()` (periodic + on-demand `REPORT` command) is the liveness signal. The Grafana Schedule-state panel reads `status`.
- [ ] **TODO (sensor cleanup)**: for backward compatibility the `sensor` payload still carries `sched_mode`/`sched_ovr`/`sched_exc`/`exc1_*`/`exc2_*` (duplicated from `status`). After **all** devices run ≥ v1.1.15 and Grafana reads `status` exclusively, remove those keys from `printValues()` in `src/app/app.cpp` and drop the corresponding `if (...)` lines in `fn_sensor` (`backend/nodered/flows.json`). Until then, keep them — removing them now would break the current backend.
- [x] **11. Custom web UI (Option B)**: `backend/webui/` is a static SPA (nginx) that controls devices **without** Node-RED — it connects to **HiveMQ Cloud over MQTT-WebSocket** (`wss://<broker>:8884/mqtt`, same `aircaredevice` creds) and publishes the identical command contract to `AirCare/inCommands/<mac>` / `/broadcast` (RELAY ON/OFF/AUTO, EXCEPTION [from/to/state], EXCEPTION_CLEAR, REPORT, UPDATE, REBOOT). It embeds the Grafana dashboard (`GF_ALLOW_EMBEDDING=true`) so panels + control live in one page. The `webui` Docker service serves it on `:8080`; `entrypoint.sh` templates `config.js` from env vars at start (no secrets in the image). **Node-RED's `/ui` dashboard (:1880) is kept as a fallback.** MQTT-WS connect + publish verified end-to-end against HiveMQ Cloud.
