...
int8_t read8bitSigned (uint8_t offsetReg) {   // Done!
  ...
  // The Sunrise needs a brief settle after the register-address write before
  // the repeated-start read; without it the requestFrom() wedges for the full
  // Wire timeout (~9s) on a noisy bus, blocking loop() — the exact stall that
  // tripped the task WDT. 250us is enough to clear it (measured on the rig).
...
int16_t read16bitSigned (uint8_t offsetReg) {         // DONE!
  ...
  Wire.write(offsetReg);
  Wire.endTransmission(false);
  // See read8bitSigned: the Sunrise wedges requestFrom() without a short settle
  // after the register-address write on a noisy bus.
  delayMicroseconds(1000);
...
boolean sunrise::readPowerDownData() {
  ...
  Wire.endTransmission(false);
  delayMicroseconds(1000); // same settle as the other reads (noisy-bus wedge fix)
